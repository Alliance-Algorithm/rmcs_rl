...
namespace {
    ...
    double parse_double(const std::string& text, const std::string& context) {
        try {
            ...
            throw std::invalid_argument(
                "RlBridge: '" + text + "' is not a number (" + context + ")");
        }
    }
    ...
    std::int64_t parse_integer(const std::string& text, const std::string& context) {
        try {
            ...
            throw std::invalid_argument(
                "RlBridge: '" + text + "' is not an integer (" + context + ")");
        }
    }
    ...
    std::string pretty_type(const std::type_info& type) {
        ...
        if (type == typeid(std::size_t)) return "std::size_t";
        if (type == typeid(Eigen::Vector3d)) return "Eigen::Vector3d";
        if (type == typeid(rmcs_description::BaseLink::DirectionVector))
            return "rmcs_description::BaseLink::DirectionVector";
        if (type == typeid(Eigen::Quaterniond)) return "Eigen::Quaterniond";
        return std::string { type.name() } + " (unknown)";
    }

} // namespace

class RlBridge final : public rmcs_executor::Component, public rclcpp::Node {
...
public:
    explicit RlBridge()
        ...
        for (const auto& spec : action_specs) {
            ...
            if (!output_paths.emplace(term.output).second)
                throw std::invalid_argument("RlBridge: two action terms map to the same output "
                                            "interface '"
                    + term.output + "'");
            if (!action_indices.emplace(term.index).second)
                throw std::invalid_argument("RlBridge: duplicate action index "
                    + std::to_string(term.index) + " (each slot must appear exactly once)");
            action_terms_.push_back(std::move(term));
        }
        for (std::size_t i = 0; i < action_terms_.size(); ++i)
            if (action_terms_[i].index != i)
                throw std::invalid_argument("RlBridge: action indices must form the complete "
                                            "permutation 0.."
                    + std::to_string(action_terms_.size() - 1) + "; got index="
                    + std::to_string(action_terms_[i].index) + " at position "
                    + std::to_string(i));
        action_size_ = action_terms_.size();
        if (const auto declared = integer_or_("rl_action_size"); declared.has_value()
            && static_cast<std::size_t>(*declared) != action_size_)
            throw std::invalid_argument("RlBridge: rl_action_size=" + std::to_string(*declared)
                + " but action_terms has " + std::to_string(action_size_) + " entries");
        ...
        if (observation_specs.empty())
            throw std::invalid_argument(
                "RlBridge: required parameter 'observation_terms' is missing");
        ...
        for (auto& term : obs_terms_) {
            if (term.has_index && term.index != cursor)
                throw std::invalid_argument("RlBridge: observation term '" + term.id + "' declares "
                                            "index="
        ...
        obs_size_ = cursor;
        if (const auto declared = integer_or_("rl_obs_size"); declared.has_value()
            && static_cast<std::size_t>(*declared) != obs_size_)
            throw std::invalid_argument("RlBridge: rl_obs_size=" + std::to_string(*declared)
        ...
        if (invalid == "nan") invalid_mode_ = InvalidMode::kNaN;
        else if (invalid == "zero") invalid_mode_ = InvalidMode::kZero;
        else if (invalid == "hold") invalid_mode_ = InvalidMode::kHold;
        else
            throw std::invalid_argument("RlBridge: invalid_value must be nan|zero|hold "
        ...
        register_output(rl_base_ + "/action_age", action_age_output_,
    ...
    void before_pairing(const OutputInfoMap& output_map) override {
        for (auto& term : obs_terms_) {
            ...
            case TermKind::kJointTorque: {
                for (std::size_t j = 0; j < term.joints.size(); ++j) {
                    ...
                    term.joint_slots.push_back(acquire_slot_(
                        path, { Binding::kDouble }, true, output_map, "joint term"));
                }
                break;
            }
            case TermKind::kPath: {
                ...
                switch (term.take) {
                case Take::kScalar:
                    candidates = { Binding::kDouble, Binding::kBool, Binding::kInt,
                        Binding::kSize };
                    break;
                ...
                case Take::kVector:
                    ...
                    break;
                case Take::kGravity:
                    candidates = { Binding::kQuaternion };
                    break;
                }
        ...
        if (!enable_path_.empty())
            enable_slot_ = acquire_slot_(enable_path_, { Binding::kBool, Binding::kDouble }, false,
                output_map, "enable interface");
        ...
        for (const auto& slot : slots_)
            if (own_output_paths_.count(slot->path) != 0)
                throw std::runtime_error("RlBridge: interface \"" + slot->path
        ...
        log_layout_();
        RCLCPP_INFO(get_logger(), "contract: obs_size=%zu action_size=%zu policy_rate=%.3f Hz",
            obs_size_, action_size_, policy_rate_);
        RCLCPP_INFO(get_logger(), "layout_hash=%s (obs/action signatures below)",
        ...
        RCLCPP_INFO(get_logger(), "topics: %s/obs -> %s/action (best_effort, keep_last=1)",
    ...
    void update() override {
        ...
        if (reset_slot_ != kNoSlot) {
            ...
            if (read_unsigned_(reset_slot_, reset_count) && reset_count != last_reset_count_) {
                ...
                reset_runtime_();
                RCLCPP_INFO(get_logger(), "RMCS reset: last_action cleared (reset_count=%llu)",
        ...
        if (has_snapshot) {
            ...
            finite  = std::all_of(snapshot.action.begin(), snapshot.action.end(),
            ...
            if (snapshot.layout_hash != layout_hash_) {
                ...
                if (contract_ok_) {
                    contract_ok_ = false;
                    RCLCPP_FATAL(get_logger(),
                        "model mismatch: action model_id=%s != expected_model_id=%s",
        ...
        if (valid) last_actions_ = snapshot.action;
        ...
        *obs_seq_output_   = pub_seq_;
        ...
        if (valid != last_valid_) {
            if (valid) {
                RCLCPP_INFO(get_logger(), "valid=1 (obs_seq=%llu model_id=%s action_age=%.4f s)",
                    static_cast<unsigned long long>(snapshot.obs_seq),
                    hex16(snapshot.model_id).c_str(), age);
            } else {
                RCLCPP_WARN(get_logger(), "valid=0: %s",
...
private:
    ...
    std::optional<double> number_(const std::string& name) {
        ...
        try {
            if (!get_parameter(name, parameter)) return std::nullopt;
        } catch (const std::exception&) {
            return std::nullopt;
        }
        ...
        case rclcpp::ParameterType::PARAMETER_NOT_SET:
            return std::nullopt;
        default:
            throw std::invalid_argument("RlBridge: parameter '" + name + "' must be a number");
        }
    }
    ...
    static std::uint64_t parse_u64_(const std::string& text, const std::string& name) {
        try {
            ...
            throw std::invalid_argument("RlBridge: parameter '" + name
    ...
    bool bool_or_(const std::string& name, bool fallback) {
        ...
        if (parameter.get_type() == rclcpp::ParameterType::PARAMETER_NOT_SET) return fallback;
    ...
    std::vector<std::string> string_array_or_(const std::string& name) {
        ...
        try {
            if (!get_parameter(name, value)) return { };
        } catch (const std::exception&) {
            throw std::invalid_argument(
    ...
    void load_joint_config_() {
        ...
        if (!joint_names_.empty() && joint_base_path_.empty())
            throw std::invalid_argument(
                "RlBridge: joint_base_path is required when joint_names is set");
        ...
        for (const auto& name : list_parameters({ "joint_interface_overrides" }, 10).names) {
            ...
            if (separator == std::string::npos)
                throw std::invalid_argument("RlBridge: joint_interface_overrides entry '" + name
            ...
            if (field == "angle") angle_suffix_[joint] = value;
            else if (field == "velocity") velocity_suffix_[joint] = value;
            else if (field == "torque") torque_suffix_[joint] = value;
            else
                throw std::invalid_argument("RlBridge: joint_interface_overrides field '" + field
    ...
    static const char* binding_name_(Binding binding) {
        ...
        case Binding::kDirectionVector:
            return "BaseLink::DirectionVector";
        case Binding::kQuaternion:
    ...
    std::size_t acquire_slot_(const std::string& path, const std::vector<Binding>& candidates,
        bool required, const OutputInfoMap& output_map, const char* context) {
        const auto output = output_map.find(path);
        if (output == output_map.end()) {
            if (!required) return kNoSlot;
            throw std::runtime_error("RlBridge: required input interface \"" + path
        ...
        if (output->second.kind != rmcs_executor::InterfaceKind::Normal)
            throw std::runtime_error("RlBridge: input interface \"" + path
        ...
        if (!found) {
            ...
            throw std::runtime_error("RlBridge: cannot bind observation interface \"" + path
                + "\": producer declares \"" + pretty_type(producer_type)
                + "\" but the term accepts { " + join(expected, ", ")
    ...
    static std::map<std::string, std::string> parse_tokens_(const std::string& spec) {
        ...
        for (const auto& token : split_whitespace(spec)) {
            ...
            if (equals == std::string::npos || equals == 0)
                throw std::invalid_argument("RlBridge: term token '" + token
                    + "' must be key=value (term '" + spec + "')");
    ...
    static void validate_tokens_(const std::map<std::string, std::string>& tokens,
        const std::vector<std::string>& allowed, const std::string& spec) {
    ...
    static double double_token_(const std::map<std::string, std::string>& tokens,
        const std::string& key, double fallback, const std::string& spec) {
    ...
    static bool bool_token_(const std::map<std::string, std::string>& tokens,
        const std::string& key, bool fallback, const std::string& spec) {
        const auto token = tokens.find(key);
        if (token == tokens.end()) return fallback;
        return parse_boolean(token->second, key + " in term '" + spec + "'");
    }

    static std::string string_token_(const std::map<std::string, std::string>& tokens,
    ...
    static void parse_clip_(const std::map<std::string, std::string>& tokens,
        const std::string& spec, bool& has_clip, double& clip_min, double& clip_max) {
        const auto clip = tokens.find("clip");
        if (clip == tokens.end()) return;
    ...
    static void parse_index_(const std::map<std::string, std::string>& tokens,
        const std::string& spec, bool& has_index, std::size_t& index) {
    ...
    static std::vector<std::size_t> parse_index_list_(
        const std::string& text, std::size_t limit, const std::string& what) {
        std::vector<std::size_t> indices;
        for (const auto& piece : split_by(text, ',')) {
            if (piece.empty())
                throw std::invalid_argument(
                    "RlBridge: empty entry in " + what + " '" + text + "'");
            const auto range = piece.find("..");
            if (range == std::string::npos) {
                const auto index = parse_integer(piece, what);
                if (index < 0 || static_cast<std::size_t>(index) >= limit)
                    throw std::invalid_argument("RlBridge: " + what + " index "
                        + std::to_string(index) + " out of range [0, " + std::to_string(limit)
                        + ")");
                ...
                if (first < 0 || last < first || static_cast<std::size_t>(last) >= limit)
                    throw std::invalid_argument("RlBridge: invalid " + what + " range '" + piece
    ...
    ObsTerm parse_obs_term_(const std::string& spec) {
        ...
        if (type == "joint_pos" || type == "joint_vel" || type == "joint_torque") {
            ...
            for (const auto joint : term.joints) {
                if (term.kind == TermKind::kJointPos && term.relative) {
                    ...
                    if (!base.has_value())
                        throw std::invalid_argument("RlBridge: joint_pos relative term '" + spec
            ...
            validate_tokens_(tokens,
                { "type", "joints", "relative", "zero", "index", "scale", "clip", "name" }, spec);
        ...
        if (type == "last_action") {
            ...
            if (const auto indices = tokens.find("indices"); indices != tokens.end())
                term.action_indices = parse_index_list_(indices->second, action_size_, "last_action");
            else
        ...
        if (path == tokens.end() || path->second.empty())
            throw std::invalid_argument("RlBridge: term requires path=... (term '" + spec
        ...
        if (transform == "projected_gravity") {
            if (!take.empty() && take != "gravity")
                throw std::invalid_argument(
                    "RlBridge: transform=projected_gravity conflicts with take=" + take
                    + " (term '" + spec + "')");
            ...
            if (!type.empty()) {
                if (type != "quaternion")
                    throw std::invalid_argument("RlBridge: projected_gravity requires a "
            ...
            throw std::invalid_argument(
                "RlBridge: transform=gravity is not supported; use transform=projected_gravity "
                "(term '"
                + spec + "')");
        } else if (!transform.empty()) {
            throw std::invalid_argument("RlBridge: unknown transform='" + transform + "' (term '"
                + spec + "')");
        } else if (take.empty()) {
            ...
            throw std::invalid_argument("RlBridge: unknown take='" + take
        ...
        if (!type.empty()) {
            ...
            if (type == "scalar" || type == "double") {
                ...
                throw std::invalid_argument("RlBridge: unknown type='" + type + "' (term '" + spec
                    + "')");
            }
            const bool scalar_binding = term.binding == Binding::kDouble
                || term.binding == Binding::kBool || term.binding == Binding::kInt
                || term.binding == Binding::kSize;
            const bool vector_binding = term.binding == Binding::kVector3
                || term.binding == Binding::kDirectionVector;
            if (term.take == Take::kScalar && !scalar_binding)
                throw std::invalid_argument("RlBridge: type=" + type
            ...
            if ((term.take == Take::kComponent || term.take == Take::kVector) && !vector_binding)
                throw std::invalid_argument("RlBridge: take=" + take + " needs a vector interface "
                                            "(type=vector3|direction_vector), got type="
                    + type + " (term '" + spec + "')");
            if (term.take == Take::kGravity && term.binding != Binding::kQuaternion)
                throw std::invalid_argument(
                    "RlBridge: transform=projected_gravity needs type=quaternion (term '" + spec
                    + "')");
        }
    ...
    ObsTerm finish_common_(ObsTerm term, const std::map<std::string, std::string>& tokens,
        const std::string& spec) {
        parse_index_(tokens, spec, term.has_index, term.index);
        term.scale = double_token_(tokens, "scale", 1.0, spec);
        if (term.scale == 0.0)
            throw std::invalid_argument(
                "RlBridge: scale=0 is not allowed (term '" + spec + "')");
        ...
        if (default_value != tokens.end()) {
            if (!(term.kind == TermKind::kPath && term.take == Take::kScalar))
                throw std::invalid_argument("RlBridge: default= is only valid for scalar path terms "
                                            "(term '"
                    + spec + "')");
            term.default_value = parse_double(default_value->second, "default");
            if (!is_finite(term.default_value))
                throw std::invalid_argument("RlBridge: default must be finite (term '" + spec + "')");
    ...
    std::string obs_layout_signature_() const {
        ...
        for (const auto& term : obs_terms_) {
            signature += "|" + term.id;
            if (term.scale != 1.0) signature += "*" + format_number(term.scale);
    ...
    void log_layout_() const {
        ...
        for (const auto& term : obs_terms_) {
            ...
            if (term.has_default) line << " default=" << format_number(term.default_value);
            if (term.has_clip)
                line << " clip=" << format_number(term.clip_min) << ":"
                     << format_number(term.clip_max);
            if (!term.path.empty()) {
                line << " <- " << term.path;
                if (term.slot != kNoSlot) line << " (" << binding_name_(slots_[term.slot]->binding)
                                              << ")";
                else line << " (default)";
            }
        ...
        for (const auto& term : action_terms_) {
            ...
            if (term.scale != 1.0) line << " scale=" << format_number(term.scale);
            RCLCPP_INFO(get_logger(), "%s", line.str().c_str());
        }
    }
    ...
    bool read_double_(std::size_t slot, double& value) const {
        ...
        case Binding::kDouble:
            if (!entry.double_value->ready()) return false;
            value = **entry.double_value;
            return true;
        case Binding::kBool:
            if (!entry.bool_value->ready()) return false;
            value = **entry.bool_value ? 1.0 : 0.0;
            return true;
        case Binding::kInt:
            if (!entry.int_value->ready()) return false;
            value = static_cast<double>(**entry.int_value);
            return true;
        case Binding::kSize:
            if (!entry.size_value->ready()) return false;
    ...
    bool read_unsigned_(std::size_t slot, std::uint64_t& value) const {
        double raw = 0.0;
        if (!read_double_(slot, raw)) return false;
        if (!is_finite(raw) || raw < 0.0) return false;
        value = static_cast<std::uint64_t>(raw);
        return true;
    }

    bool read_enable_() const {
        if (enable_slot_ == kNoSlot) return enable_default_;
        double raw = 0.0;
        if (!read_double_(enable_slot_, raw)) return enable_default_;
        return raw != 0.0;
    }

    bool build_observation_(std::vector<double>& obs) const {
        obs.assign(obs_size_, 0.0);
        const auto push = [&obs](std::size_t index, double value, double scale, bool has_clip,
                              double clip_min, double clip_max, bool& ok) {
            double result = value * scale;
            if (has_clip) result = std::clamp(result, clip_min, clip_max);
            if (!is_finite(result)) ok = false;
            obs[index] = result;
        };
        ...
        for (const auto& term : obs_terms_) {
            ...
            case TermKind::kPath: {
                if (term.slot == kNoSlot) {
                    push(term.index, term.default_value, term.scale, term.has_clip, term.clip_min,
                        term.clip_max, ok);
                    break;
                }
                ...
                if (term.take == Take::kScalar) {
                    double value = 0.0;
                    if (!read_double_(term.slot, value)) return false;
                    push(term.index, value, term.scale, term.has_clip, term.clip_min, term.clip_max,
                        ok);
                } else if (term.take == Take::kComponent) {
                    double value = 0.0;
                    if (entry.binding == Binding::kVector3) {
                        if (!entry.vector3_value->ready()) return false;
                        value = (**entry.vector3_value)[term.component];
                    } else {
                        if (!entry.direction_vector_value->ready()) return false;
                    ...
                    if (entry.binding == Binding::kVector3) {
                        ...
                        for (std::size_t i = 0; i < 3; ++i)
                            push(term.index + i, value[static_cast<Eigen::Index>(i)], term.scale,
                    ...
                    if (!entry.quaternion_value->ready()) return false;
                    const Eigen::Vector3d gravity =
                        **entry.quaternion_value * Eigen::Vector3d(0.0, 0.0, -1.0);
                    for (std::size_t i = 0; i < 3; ++i)
                        push(term.index + i, gravity[static_cast<Eigen::Index>(i)], term.scale,
                            term.has_clip, term.clip_min, term.clip_max, ok);
                }
                break;
            }
            case TermKind::kJointPos: {
                for (std::size_t j = 0; j < term.joints.size(); ++j) {
                    double value = 0.0;
                    if (!term.zero) {
                        if (!read_double_(term.joint_slots[j], value)) return false;
                        if (term.relative) value -= term.joint_defaults[j];
                    }
                    push(term.index + j, value, term.scale, term.has_clip, term.clip_min,
                        term.clip_max, ok);
                }
                break;
            }
            ...
            case TermKind::kLastAction: {
                for (std::size_t j = 0; j < term.action_indices.size(); ++j)
                    push(term.index + j, last_actions_[term.action_indices[j]], term.scale,
                        term.has_clip, term.clip_min, term.clip_max, ok);
                break;
            }
            case TermKind::kConstant: {
                for (std::size_t j = 0; j < term.constants.size(); ++j)
                    push(term.index + j, term.constants[j], term.scale, term.has_clip, term.clip_min,
                        term.clip_max, ok);
                break;
            }
            }
    ...
    void on_action_(rmcs_rl::msg::Action::UniquePtr message) {
        ...
        if (message->action.size() != action_size_) {
            RCLCPP_ERROR_THROTTLE(get_logger(), *get_clock(), 1000,
                "ignoring action with %zu values (expected %zu)", message->action.size(),
                action_size_);
            return;
        }
    ...
    bool try_read_action_(ActionSnapshot& snapshot) {
        for (int attempt = 0; attempt < 4; ++attempt) {
            ...
            snapshot = incoming_;
            std::atomic_thread_fence(std::memory_order_acquire);
            if (action_sequence_.load(std::memory_order_relaxed) == before) return true;
        }
        return false;
    }

    double obs_age_of_(std::uint64_t obs_seq, std::chrono::steady_clock::time_point now) const {
        if (!pub_started_) return std::numeric_limits<double>::quiet_NaN();
        if (obs_seq == pub_seq_)
    ...
    void write_actions_(bool valid, const ActionSnapshot& snapshot) {
        for (std::size_t i = 0; i < action_terms_.size(); ++i) {
            ...
            if (valid) {
                ...
                if (term.has_clip) value = std::clamp(value, term.clip_min, term.clip_max);
                written_[i] = value;
            } else {
                ...
                case InvalidMode::kNaN:
                    ...
                    break;
                case InvalidMode::kZero:
                    value = 0.0;
                    break;
                case InvalidMode::kHold:
                    value = written_[i];
                    break;
                }
            }
    ...
    std::string invalid_reason_(bool enabled, bool contract_ok, bool has_snapshot, bool fresh,
        bool seq_ok, bool finite) const {
        if (!contract_ok) return "contract/model mismatch (latched; restart or re-enable)";
        if (!enabled) return "disabled by enable interface";
        if (!has_snapshot) return "no action received yet";
        if (!finite) return "action contains non-finite values";
        if (!seq_ok) return "action answers an obs frame older than the previous one";
        if (!fresh) return "action is stale (age > max_action_age)";
        return "unknown";
    }
...
PLUGINLIB_EXPORT_CLASS(rmcs::rl::RlBridge, rmcs_executor::Component)
