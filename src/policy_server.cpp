        ...
        if (!piece.empty()) {
            try {
                ...
                throw std::invalid_argument(
                    std::string{"policy_server: metadata "} + what + " has a non-number '" + piece
                    + "'");
            }
        }
...
double parse_clip_metadata(const std::string& text, const char* key) {
    ...
    if (!std::isfinite(*value) || *value <= 0.0)
        throw std::runtime_error(
            std::string{"policy_server: metadata "} + key + " must be finite and > 0, got " + text);
    return *value;
}
...
public:
    PolicyServer()
        ...
        if (!obs_layout || obs_layout->empty() || !actions_layout || actions_layout->empty())
            throw std::runtime_error(
                "policy_server: model '" + resolved_model_path_
                + "' is missing metadata 'rmcs_obs_layout' / 'rmcs_actions_layout'; stamp it with "
                  "tool/stamp_layout_metadata.py before deploying (see "
                  "planning/docs/bridge-design.md §6)");
        ...
        if (const auto declared = inference_.metadata("policy_layout_hash");
            ...
            try {
                ...
                throw std::runtime_error(
                    "policy_server: metadata 'policy_layout_hash' is not a "
                    "hex string: '"
                    + *declared + "'");
            }
            if (declared_hash != layout_hash_)
                throw std::runtime_error(
                    "policy_server: metadata 'policy_layout_hash'=" + *declared
                    + " does not match the hash computed from the layout metadata ("
        ...
        RCLCPP_INFO(
            get_logger(), "  layout_hash   : %s  (bridge must log the same value)",
            hex16(layout_hash_).c_str());
        if (!obs_mean_.empty())
            RCLCPP_INFO(
                get_logger(), "  normalization : mean/std from metadata, obs_clip=%s",
    ...
    void load_normalization_() {
        ...
        if (from_metadata) {
            ...
            if (!obs_mean_.empty() && obs_mean_.size() != obs_size_)
                throw std::runtime_error(
                    "policy_server: metadata rmcs_obs_mean has " + std::to_string(obs_mean_.size())
                    + " values, expected " + std::to_string(obs_size_));
            if (!obs_std_.empty() && obs_std_.size() != obs_size_)
                throw std::runtime_error(
                    "policy_server: metadata rmcs_obs_std has " + std::to_string(obs_std_.size())
                    + " values, expected " + std::to_string(obs_size_));
            if (!obs_mean_.empty() && !obs_std_.empty()) {
                for (const double sigma : obs_std_)
                    if (!(sigma > 0.0) || !std::isfinite(sigma))
                        throw std::runtime_error(
                            "policy_server: metadata rmcs_obs_std must be "
                            "finite and > 0");
            } else {
                obs_mean_.clear();
                obs_std_.clear();
        ...
        if (obs_clip_param >= 0.0)
            obs_clip_ = obs_clip_param;
        const double action_clip_param = number_or_("action_clip", -1.0);
        if (action_clip_param >= 0.0)
            action_clip_ = action_clip_param;
    ...
    void on_observation_(rmcs_rl::msg::Observation::UniquePtr message) {
        ...
        if (message->obs.size() != obs_size_) {
            RCLCPP_ERROR_THROTTLE(
                get_logger(), *get_clock(), 1000, "obs has %zu values, model expects %zu; ignoring",
                message->obs.size(), obs_size_);
            ++rejected_count_;
            return;
        }
        ...
        for (std::size_t i = 0; i < obs_size_; ++i) {
            ...
            if (!std::isfinite(value)) {
                RCLCPP_WARN_THROTTLE(
                    get_logger(), *get_clock(), 1000, "obs[%zu] is not finite; ignoring frame", i);
                ++rejected_count_;
                return;
            }
        ...
        if (!inference_.run(obs_buffer_, action_buffer_)) {
            RCLCPP_ERROR_THROTTLE(
                get_logger(), *get_clock(), 1000, "ONNX Runtime rejected the obs frame; ignoring");
            ++rejected_count_;
            return;
        }
        ...
        for (std::size_t i = 0; i < action_size_; ++i) {
            ...
            if (action_clip_.has_value())
                value = std::clamp(value, -*action_clip_, *action_clip_);
